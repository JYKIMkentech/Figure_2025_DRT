function [gamma_lower, gamma_upper, gamma_resample_all, gamma_avg] = bootstrap_uncertainty(t, ik, V_sd, lambda, n, ~, dur, OCV, R0, num_resamples)
    ...
    %   gamma_avg           - Average of all bootstrap gamma estimates
    ...
    for b = 1:num_resamples
        ...
        % 여기서 DRT_estimation 함수를 통해 부트스트랩된 gamma를 추정
        [gamma_resample, ~, ~, ~, ~] = DRT_estimation(t_resampled_sorted, ...
                                                      ik_resampled_sorted, ...
                                                      V_sd_resampled_sorted, ...
                                                      lambda, n, dt_resampled, ...
                                                      dur, OCV, R0);
    ...
    % 추가: 각 빈(bin, frequency)별로 부트스트랩된 gamma의 평균을 계산
