...
%% (6) Reflect back to AS_data (for문 to avoid mismatch)
...
%% (7) Plot in Subplot(2 x 5)
%  여기서는 시나리오가 10개라고 가정
%  필요시 num_cols=5; num_rows=ceil(num_scenarios/5); 로 일반화 가능
num_cols = 5;
num_rows = 2;
...
for s = 1:num_scenarios
    ...
    fill([th_s, fliplr(th_s)], ...
         [gl_s, fliplr(gu_s)], ...
         plotColor, 'FaceAlpha',0.3, 'EdgeColor','none');
    hold on;
...
%% (8) gamma_est가 (gamma_lower, gamma_upper) 사이에 있는지 판별
...
%  - eval trick: AS_name이 "AS1_1per_new" 라면, "AS1_1per_new = AS_data;"
%    & save(..., 'AS1_1per_new')
